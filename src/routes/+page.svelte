<script lang="ts">
    ...
    // Mock data that will eventually come from the LLM
    // For this test, let's use a dark blue-grey and a light grey
    let mockColors = ["#1A202C", "#A0AEC0"];
    let mockSounds = ["heavy rain", "distant thunder"];
...
<main
    class="h-screen w-screen bg-black text-white flex flex-col items-center justify-center font-serif p-4"
>
    {#if !submitted}
        <div class="text-center transition-opacity duration-1000 w-full">
            <h1 class="text-4xl md:text-5xl mb-12 animate-fade-in">
    ...
    {:else}
        <!-- When submitted, we render the P5Canvas and pass the mock data to it -->
        <P5Canvas {prompt} colors={mockColors} sounds={mockSounds} />
